...
import { RequestEnum, ResultEnum, ContentTypeEnum } from '/@/enums/http';
import { isString, isUndefined, isNull, isEmpty, isUrl } from '/@/utils/is';
import { setObjToUrlParams, deepMerge } from '/@/utils';
import { joinTimestamp, formatRequestDate } from './helper';
import { AxiosRetry } from '/@/utils/axios/retry';
import axios from 'axios';
import { createXRequestID } from '../index';
...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    // 这里逻辑可以根据项目进行修改
    const hasSuccess = !isEmpty(data);
    if (hasSuccess) {
      let successMsg = 'success message';

      if (isNull(successMsg) || isUndefined(successMsg) || isEmpty(successMsg)) {
        successMsg = `sys.api.operationSuccess`;
      }
      if (options.successMessageMode === 'modal') {
        createSuccessModal({ title: 'sys.api.successTip', content: successMsg });
  ...
  requestInterceptors: (config, options) => {
    ...
    // UUID
    (config as Recordable).headers['X-Request-ID'] = createXRequestID();
    nProgress.start();
    return config;
  },
